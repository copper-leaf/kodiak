...
class JavadocJsonRunnerTest {
    ...
    private fun initProjectDirs() {
        cacheDir = File("../javadoc-json-formatter/build/javadoc/cache").canonicalFile.toPath()
        outputDir = File("../javadoc-json-formatter/build/javadoc/output").canonicalFile.toPath()
        outputDir.toFile().deleteRecursively()
    ...
    fun testRunningJavadoc() {
        try {
            val rootDoc = javadocRunner.getRootDoc(
                listOf(File("../javadoc-json-formatter/src/test/java").canonicalFile.toPath()),
                outputDir
