...
class DokkaJsonRunnerTest {
    ...
    private fun initProjectDirs() {
        cacheDir = File("../dokka-json-formatter/build/dokka/cache").canonicalFile.toPath()
        outputDir = File("../dokka-json-formatter/build/dokka/output").canonicalFile.toPath()
        outputDir.toFile().deleteRecursively()
    ...
    fun testRunningDokka() {
        try {
            val rootDoc = dokkaRunner.getRootDoc(
                listOf(
                    File("../dokka-json-formatter/src/test/java").canonicalFile.toPath(),
                    File("../dokka-json-formatter/src/test/kotlin").canonicalFile.toPath()
                ),
                outputDir
